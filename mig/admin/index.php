...
if ( $do==0 ) {
	...
	echo '<li><a href="?do=1">+ ADD NEW USER</a></li>';
	echo '</ul>';
} else {
	$sespost=null;$styleis=$bad='';
	$_SESSION['mig_last'] = DLC_MGRT.'/admin';
	...
	echo '<li><label>MIG ID:</label>'.($do==1?'<select name="un">'.getDrop($un).'</select>':$un.'<input type="hidden" name="un" value="'.$un.'" />').'</li>';
	echo '<li><label>Password:</label><input type="password" '.$styleis.' name="pw" class="txt ex" value="" /><input type="hidden" name="scop" value="MIG" /></li>';

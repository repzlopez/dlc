...
$link		='mycart';
$linkname		='myCart';
$tab			=array('pv'=>'','downline'=>'','bonus'=>'','order'=>'','mgrlist'=>'','bbb'=>'','pool'=>'','slots'=>'');
...
list($distdata,$ppv)=getDistributor(DIST_ID);
...
function getData($dsdid,$ppv,$lvl) {
	...
	switch(GET_PAGE) {
		...
		case 'pool': $x .= getPool($dsdid);break;
...
function getManagerList() {
	...
	$m_list='';$x='';
	// $qry="
	// 	SELECT DISTINCT bddids,dsfnam,dslnam,bhdid,bhppv,bhnpv,bhelev,bhqumr
	// 	FROM bodstp
	// 	INNER JOIN distributors ON dsdid=bddids
	// 	LEFT JOIN bohstp ON bhdid=bddids
	// 	WHERE dsdid=bddids
	// 	AND bddids NOT LIKE 'DLC%'
	// 	AND bdyy='$dyr' AND bhpyr='$dyr'
	// 	AND bdmm='$dwk' AND bhpmo='$dwk'
	// 	AND (bdtype='L' OR (bdtype='B' AND (bddids='".EDDY."' OR bddids='".RICK."')))
	// 	ORDER BY IF(bddids RLIKE '^[a-z]',1,2),bddids
	// ";
	$qry="
...
function getLastUpdate() {
	...
	$rs=mysqli_query($con,"SELECT $get FROM $tbl ORDER BY $get DESC LIMIT 1") or die(mysqli_error($con));

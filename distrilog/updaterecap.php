...
function getDistributor($id, $wk, $yr) {
	...
	$rs  = $con->query($qry) or die($con->error);
	$rw  = $rs->fetch_array();
...
function getBonusRec($id, $wk, $yr) {
	...
	$con = SQLi('distributor');
	$rs1 = $con->query( $qry . "'$wk' AND bdyy='$yr'") or die($con->error);

	if( $rs1->num_rows > 0 ) {
		$rw1 = $rs1->fetch_array();
		$now = $rw1['bdbpct'] * 100;

		$rs2 = $con->query($qry . (($wk > 1) ? "'" . ($wk - 1) . "' AND bdyy='$yr'" : "'52' AND bdyy='" . ($yr - 1) . "'")) or die($con->error);
		$rw2 = $rs2->fetch_array();
		$bak = $rw2['bdbpct'] * 100;
	}

...
$ppv=0;$npv=0;$gpv=0;$lev=0;$ndpv=0;
...
$qry="
     SELECT CONCAT(dslnam,', ',dsfnam,' ',dsmnam) nam,dssid,dstin,$x $qryAllowSponsorD,$qryAllowSponsorO,
...
if(isset($id1)){
     ...
     foreach(explode('|',$allid) as $k){
          if($k!=$id) $x.='<span><a href="?page_id=384&otherid='.$k.'" class="s4">'.$k.'</a></span>';
     }
     $x.='</div></li>';
}
$x.='<li><label>Sponsor:</label><br><div><span>'.$dssid.'</span><span>'.ucwords(strtolower($sname)).'</span></div></li>';
...
$x.='<li><hr></li>';
$x.='<li><label>Referral Link:</label><br><div><code>'.$reflink.'</code> <a href="#" class="reflink" rel="'.$reflink.'">Copy Link</a></div></li>';

if( $d_allow || $o_allow || isset($id1) || $refer==$over ){}
else{
     $x.='<li><hr></li>';
     $x.='<li><h4 class="good">RESELLER</h4><div>You need to accumulate '.$minpv.'PV in one cut-off to become a distributor</div></li>';
}

if($dstin==''){
     $x.='<li><hr></li>';
     $x.='<li><h4 class="bad">NO TIN</h4><div>Kindly update your TIN as required by BIR. NO TIN, NO VOLUME BONUS.</div></li>';
}
...
$x.='<li><hr></li>';
$x.='<li><input type="button" rel="/distrilog/mypage.php" class="link" value="Dashboard"> <input type="button" rel="'.get_permalink($profile->ID).'?i='.$dlcuser['id'].'" class="link" value="Profile"> <input type="button" rel="/distrilog/myorders.php" class="link" value="Orders"> '.$cartbtn.'</li>';
$x.='</ul>';

$(document).ready(function(){
			...
			if(pascur==''||pasold==''||pasnew==''){
				...
				if(x){
					$.ajax({
						...
						success: function(n){
							switch(n.substr(0,1)){
								case '1':msg+=n.substr(1);break;
								default:stat='good';msg=n.substr(1);
	...
	if($('.orders').length>0){
		var kartht=82;olddist='';oldname='';oldsearch='';
		updateProducts();
		updateOrder(null,0);
		loadCart();
		...
		$('#gos_tab').live('click',function(){
			if($(this).text()=='CHECKOUT'){
				if(testOrders()){
					...
					sumChange();
				}else{ alert('You forgot your orders!');return false; }
			}else{
...
function sortProducts(){
	...
	$.ajax({
		type: 'POST',
		url: 'updateproducts.php',
		data: {'sortonly':true,'sortorder':sort},
		beforeSend: function(){
			$('#loadgif').hide();
			lockProp(1);
...
function testSubmit(){
	...
	$('#uldist input.txt,#batpaym input.txt').each(function(){
		if(($.trim($(this).val())==''||$.trim($(this).val())=='NOT FOUND')&&$(this).attr('name')!='gc_conf'){
			fail=1;$(this).css({borderColor:'#f00'});
		}
	});
	if(fail){

...
function loadRemit($monyr) {
	...
	if($do==0) {
		...
		switch($tabid) {
			...
			default:
				if(isset($sum)) {
					...
					echo '<span class="s3 ct">'.(IS_PCM?'Confirmation':'Date Paid').'</span>';
		...
		$_SESSION['pcm_last']=htmlspecialchars($_SERVER['HTTP_REFERER']);
		...
		if($do==1) {
			...
			if(mysqli_num_rows($rs)>0) {
				...
				echo '<li><span class="s1 lt">PCM:</span><span class="s5">'.$brn.'</span> <span class="s1 lt">Trans:</span><span class="s3">'.$item.'</span></li>';
				...
				}echo '</ul>'.(IS_PCM&&$unp?'<a href="void.php" class="back voidorder" id="download">VOID ORDER</a>':'');
				$src=DLC_PCRT.'/remit/scan/'.$item.'.jpg';
				$_SESSION['editorder']=(!IS_PCM&&!$rw['paystat']&&!$rw['replen']&&!$rw['status'])?'<a href="printorders.php?gcref='.$item.'" class="back edit_order" id="download">EDIT ORDER</a>':'';
			}
			if($prt) {
...
function listTrans($id,$monyr) {
	...
	while($rw=mysqli_fetch_assoc($rs)) {
		...
		if($old==$new) {
			...
			if(IS_PCM) {$msg.='<li class="nobg"></li><li class="nobg"><span class="blue s6">PCM # '.$new.'</span></li>';}
			if($old!='') { $ttsham=0;}
		}$old=$new;
		$msg.='<li rel="'.$rfno.'">';
		$msg.='<span class="s4">'.(IS_PCM?($remt?$chck:''):($remt?($id=='paid'?$chck:''):$chck)).'<a href="?p=remit&do=1&i='.$rfno.'">'.$rfno.'</a></span>';
		$msg.='<span class="s3 ct">'.(IS_PCM?$conf:$det).'</span>';
	...
	$msg.=($id=='unpaid')?$total:'';
	$batbttn=($id=='unpaid'?'<a href="#" class="back batclick" rel="'.(IS_PCM?$cid:$bid).'" id="download">BATCH '.(IS_PCM?$c:$b).'</a>':'');
	$addconf=IS_PCM&&$id=='unpaid'?'<a href="#" class="back dobatch" id="batconf">CONFIRM</a>':'';
	...
	if($id=='unpaid') $msg.=IS_PCM&&$num>0?'':$paym;
	return $msg;
}

function listReplenish() {
	$old='-';$msg='';$btn='';
	$xqry=IS_PCM?'':"WHERE code LIKE '".LOGIN_BRANCH."%'";
		...
		while($rw=mysqli_fetch_assoc($rs)) {
			...
			if($old!=$brn) {
				...
				if(IS_PCM) {
					...
					$msg.='<li class="hdr"><span class="blue">PCM # '.$brn.'</span>';
...
function getBtn($brn) {
	$btn ='<li class="nobor">';
	$btn.='<a href="#" rel="'.$brn.'" class="back" id="download">'.(IS_PCM?'CONFIRM':'REPLENISH').'</a>';
	$btn.='</li></ul>';
	return $btn;
}
	...
	if($sum) {//IS_PCM&&
		...
		while($rw=mysqli_fetch_assoc($rs)) {
			...
			foreach($arr as $i=>$v) {
				...
				$oppv=$v5*$v3;$tppv+=$oppv;
				$oamt=$v4*$v3;$tamt+=$oamt;
			...
			$msg.='</table><br />';
		}$subsidy=($rtpov*(SUBSIDY/100));$PCM=$subsidy-($subsidy*.1);
		...
		if(mysqli_num_rows($rs)>0) {
			...
			// $msg.='</ul><br /><ul class="paysummary"><li><span class="s3"></span><span class="s5">Subsidy '.SUBSIDY.'% / Less 10% Tax</span><span class="s4 rt">'.number_format($subsidy,2).'</span><span class="s4 rt">'.number_format($PCM,2).'</span></li>';
		}$msg.='</ul>';
		return ''.$msg.'';
	}else{
		global $hilite;$old='';
		...
		while($rw=mysqli_fetch_assoc($rs)) {
			...
			if($old!=$new) {
				...
				$msg.='<li class="remittance'.$hili.'" rel="'.$reid.'"><span class="s4"><a href="'.DLC_PCRT.'/remit/?tab=remittance&sum='.$reid.'">'.$reid.'</a></span>';//(IS_PCM?DLC_PCRT.'/remit/?tab=remittance&sum='.$reid:'#')
				$msg.='<span class="s3 ct">'.$date.'</span>';
				$msg.='<span class="s3"></span><span class="s3"></span>';
				$msg.='<span class="s3 rt">'.number_format($red['pay'],2).'</span></li>';
			}$old=$new;

			$msg.='<li class="'.$reid.$hide.'"><span class="s4"></span><span class="s3"></span>';
			$msg.='<span class="s3 rt">'.$type.'</span>';
			$msg.='<span class="s3 rt">'.number_format($pamt,2).'</span>';
...
function getNumbers($id) {
	...
	if($id=='unpaid') {
		$fil=$tab[$id];
		$notglob=(IS_PCM?"AND remitid<>''":'');
		$qry="SELECT * FROM tblorders WHERE ".substr($tab[$id],4,strlen($tab[$id])).(IS_PCM?"$fil $notglob ":" AND refno LIKE '".LOGIN_BRANCH."%'");
		$con=SQLi('pcm');
		$rs=mysqli_query($con,$qry) or die(mysqli_error($con));
		$ret=mysqli_num_rows($rs);
	}return $ret;
}
?>

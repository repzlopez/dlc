	...
	if(mysqli_num_rows($rs)>0){
		while($rw=mysqli_fetch_assoc($rs)){
			...
			if($old==$brn){
				...
				$msg.='<span class="s3 rt"><input type="text" rel="'.$cod.'" class="txt s1 rt" value="'.$req.'" '.(IS_GOS?'':READONLY).' /></span></li>';
			}
	...
	if($sum){//IS_GOS&&
		...
		while($rw=mysqli_fetch_assoc($rs)){
			...
			if($old!=$new){
				...
				$msg.='<span class="s3 rt">'.number_format((double)$red['pay'],2).'</span></li>';
			}$old=$new;

			$msg.='<li class="'.$reid.$hide.'"><span class="s4"></span><span class="s3"></span>';
			$msg.='<span class="s3 rt">'.$type.'</span>';
			$msg.='<span class="s3 rt">'.number_format((double)$pamt,2).'</span>';

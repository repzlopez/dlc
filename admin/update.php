...
if( isset($submit) && $submit=='Submit' ) {
	...
	if( $do==1 ) {
		...
		if( $url=='admin' ) {
			...
			$con->query("INSERT INTO $tbl VALUES ($idata)");

		} else {
			// echo "INSERT INTO $tbl VALUES ($idata);"."\n\n";			// ##### REMOVE #####
			$con->query("INSERT INTO $tbl VALUES ($idata);");
			if( $url=='warehouse' ) $con->query("ALTER TABLE `tblstocks` ADD `w$id` INT NOT NULL;") or die(mysqli_error($con));
		}
		...
		if( $url=='admin' ) {
			...
			$con->query("UPDATE tbladmin SET status=$status WHERE id=995") or die(mysqli_error($con));

		} elseif( $url=='stocks' ) {
			$con->query("UPDATE tblstocks SET safeqty=$safe $qry");
			...
			$con = SQLi('orders');
			$con->query("UPDATE $tbl SET $udata WHERE slotid='$oldid'") or die(mysqli_error($con));
			...
			if( !$wholeline ) $con->query("UPDATE distributors SET dssid='$oldsp' WHERE dssid='$oldid'") or die(mysqli_error($con));
			echo ( $wholeline ? 'WHOLE LINE MOVE SUCCESS' :'' );
			$con->query("UPDATE distributors SET dssid='$dssid' WHERE dsdid='$oldid'") or die(mysqli_error($con));
		...
		$con->query("DELETE FROM $tbl WHERE id='$id'");
	}

	mysqli_close($con);

	if( !empty($_FILES) && isset($id) ) {
...
function setFiles($tbl,$file,$id,$setto) {
	$con = setupdb();
	$con->query("UPDATE $tbl SET $setto='$file' WHERE id='$id'");
	mysqli_close($con);
}

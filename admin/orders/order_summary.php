...
function verifyRef($ref) {
	$con = SQLi('orders');
	$rs  = $con->query("SELECT refNo FROM tblorders WHERE refNo = '$ref'");
	return ( $rs->num_rows>0 ) ? true : false;
}

function viewOrder($ref,$qry,$boxpay) {
	$con = SQLi('orders');
	$rs  = $con->query("SELECT dsdid,orders,deliBox FROM tblorders WHERE $qry") or die(mysqli_error($con));

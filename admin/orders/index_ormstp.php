...
if( $do==0 ) {
	...
	$pm_bonus = ' <a href="?p=preferred_member" id="download" data-recalc="pm_bonus" class="btn pm_bonus">PREFERRED MEMBER BONUS</a>';

	$x .= '<ul id="'.$content.'" class="list clear">';
	$x .= '<li class="rt"><a href="#" id="download" data-recalc="bomstp" class="btn recalc">RECALCULATE BOMSTP</a>'.$slots. $pm_bonus .'<span id="recalcmsg" class="s5 rt"></span></li>';
	...
	$rs = $con->query($qry);
	while( $r = $rs->fetch_assoc() ) {
		$x .= '<li rel="'.$r['oominv'].'" '.($r['oompv']!=$r['dompv']?'class="bad"':'').'>';
		$x .= '<span class="s2"><a href="?p='.$content.'&do=2&i='.$r['oominv'].'" class="s2">'.$r['ompmo'].'</a></span>';
		$x .= '<span class="s2"><a href="?p='.$content.'&do=2&i='.$r['oominv'].'" class="s2">'.$r['oominv'].'</a></span>';
		$x .= '<span class="id s4">'.strtoupper($r['omdid']).'</span>';
		$x .= '<span class="id s5">'.ucwords($r['nam']).'</span>';
		$x .= '<span class="s3 rt">'.number_format($r['oompv'],2).'</span>';
		$x .= '<span class="s3 rt">'.number_format($r['dompv'],2).'</span>';
		$x .= '<span class="s3 rt">'.formatDate($r['omodat'],'Ymd').'</span></li>';
	}
	...
	if($do==1) {
		...
		if($item!='') {
			...
			while( $r = $rs->fetch_assoc() ) foreach($r as $k=>$v) { $$k=$v;}
		...
		$rs = $con->query("SELECT * FROM $tbl WHERE ominv='$item'");
		while( $r = $rs->fetch_assoc() ) foreach($r as $k=>$v) { $$k=$v;}
	}
...
function loadCenter($db,$wk,$thu,$wed,$all=0,$yr='') {
	...
	while( $r = $rs->fetch_assoc() ) {
		...
		if($all) list( $wk, , , ) = getDatWk( substr($r['refdate'],0,4) .'-'. substr($r['refdate'],4,2) .'-'. substr($r['refdate'],-2) );

		$href = '?p=ormstp&do=1&c='.$db.'&i='.$r['refno'];
		$x .= '<li rel="'.$r['refno'].'"><span class="s2"><a href="'.$href.'" class="s2">'.sprintf("%02d",$wk).'</a></span>';
		$x .= '<span class="s2"><a href="'.$href.'" class="s2">PENDING</a></span>';
		$x .= '<span class="id s4">'.$r['dsdid'].'</span>';
		$x .= '<span class="id s5">'.ucwords(strtolower(getName($r['dsdid'],'lfm'))).'</span>';
		$x .= '<span class="s3 rt">-</span><span class="s3 rt">-</span>';
		$x .= '<span class="s3 rt">'.formatDate($r['refdate'],'Ymd').'</span></li>';
	}

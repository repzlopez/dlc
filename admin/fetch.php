...
function getWHID($id) {
	$con = SQLi('products');
	$rs  = $con->query("SELECT * FROM tblwarehouse WHERE wh='$id'") or die(mysqli_error($con));
	$rw  = $rs->fetch_array();
	return $rw['id'];
}
...
function countWHID() {
	$con = SQLi('products');
	$rs  = $con->query("SELECT count(*) i FROM information_schema.columns WHERE table_name ='tblstocks'") or die(mysqli_error($con));
	$rw  = $rs->fetch_array();
	return $rw['i'];
}
...
function getTransfers($transfers,$reciv=null,$reles=null) {
	...
	foreach ($transfers as $v) {
		...
		$dat1  = ($isasm ? '' : '<span class="s3 rt">' . (isset($reles[$i]) ? number_format((float)$reles[$i]) : '') . '</span>' . '<span class="s3 rt">' . (isset($trec[0]) ? number_format((float)$trec[0]) : '') . '</span>');
...
function populateCat($tbl, $id, $val, $distinct = '', $qry = '', $selected = '', $rel = '', $db = '') {
	$pCat = '';
	$con  = SQLi($db != '' ? $db : 'beta');
	$rs   = $con->query("SELECT $distinct FROM $tbl $qry") or die(mysqli_error($con));
